...
public class StepTracker {
    ...
    void amountStep() {
        ...
        System.out.println("Сохранили колличество шагов в нужный день");
    ...
    void printFullStat(int numberMonth) {
        for (int j = 0; j < monthToData[numberMonth].dayData.length; j++) {
            System.out.print(" " + j + " день: " + monthToData[numberMonth].dayData[j]);
        }
    ...
    int bestSeries(int numberMonth) {
        ...
        int bestSeries = 0;
        for (int j = 0; j < monthToData[numberMonth].dayData.length; j++) {
            if (monthToData[numberMonth].dayData[j] >= numberMonth) {
                series = series + 1;
